...
class Miccai2021_LDR_laplacian_unit_disp_add_AdaIn_lvl1(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, is_train=True, imgshape=(160, 192, 144),
                 range_flow=0.4,
                 num_block=5):
        ...
        self.diff_transform = DiffeomorphicTransform_unit(time_step=7)
    ...
    def forward(self, x, y, reg_code):
        ...
        warpped_inputx_lvl1_out = self.transform(x, output_disp_e0_v.permute(0, 2, 3, 4, 1), self.grid_1.to(x.device))
...
class Miccai2021_LDR_laplacian_unit_disp_add_AdaIn_lvl2(nn.Module):
    ...
    def forward(self, x, y, reg_code):
        ...
        warpped_x = self.transform(x_down, lvl1_disp_up.permute(0, 2, 3, 4, 1), self.grid_1.to(x.device))
        ...
        warpped_inputx_lvl1_out = self.transform(x, compose_field_e0_lvl1.permute(0, 2, 3, 4, 1),
                                                 self.grid_1.to(x.device))
...
class Miccai2021_LDR_laplacian_unit_disp_add_AdaIn_lvl3(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, is_train=True, imgshape=(160, 192, 144),
        ...
        self.transform = SpatialTransform_unit()
        # self.com_transform = CompositionTransform_unit().to(device)
    ...
    def forward(self, x, y, reg_code, is_train=True):
        lvl2_disp, _, _, lvl2_v, lvl1_v, lvl2_embedding = self.model_lvl2(x, y, reg_code)

        lvl2_disp_up = self.up_tri(lvl2_disp)
        warpped_x = self.transform(x, lvl2_disp_up.permute(0, 2, 3, 4, 1), self.grid_1.to(x.device))
        ...
        if is_train is True:
            warpped_inputx_lvl1_out = self.transform(x, compose_field_e0_lvl1.permute(0, 2, 3, 4, 1),
                                                     self.grid_1.to(x.device))
            return compose_field_e0_lvl1, warpped_inputx_lvl1_out, output_disp_e0_v, lvl1_v, lvl2_disp, e0
        else:
            return compose_field_e0_lvl1
...
class ResNextBlock(nn.Module):
    ...
    def __init__(self, in_planes, cardinality=32, bottleneck_width=4, stride=1):
        ...
        self.conv2 = nn.Conv3d(group_width, group_width, kernel_size=3, stride=stride, padding=1, groups=cardinality,
                               bias=bias_opt)
...
def weighted_smoothloss(y_pred, weight):
    ...
    return (torch.mean(weight[:, :, :, 1:, :] * (dx * dx)) + torch.mean(
        weight[:, :, 1:, :, :] * (dy * dy)) + torch.mean(weight[:, :, :, :, 1:] * (dz * dz))) / 3.0
...
#         #      [[1., 1., 1.],
#         #       [2., 2., 2.],
#         #       [1., 1., 1.]]], requires_grad=False).to(device)
#         sobel = torch.tensor([
#             [-0.5, 0., 0.5],
#             [-0.5, 0., 0.5],
#             [-0.5, 0., 0.5],
...
#              [[0.5, 0.5, 0.5],
#               [0.5, 0.5, 0.5],
#               [0.5, 0.5, 0.5]]], requires_grad=False)
#
...
#         Ix = F.conv3d(I_pad, self.sobel_x.to(I.device), padding=0, groups=self.channel)
#         Iy = F.conv3d(I_pad, self.sobel_y.to(I.device), padding=0, groups=self.channel)
#         Iz = F.conv3d(I_pad, self.sobel_z.to(I.device), padding=0, groups=self.channel)
#         Jx = F.conv3d(J_pad, self.sobel_x.to(I.device), padding=0, groups=self.channel)
#         Jy = F.conv3d(J_pad, self.sobel_y.to(I.device), padding=0, groups=self.channel)
#         Jz = F.conv3d(J_pad, self.sobel_z.to(I.device), padding=0, groups=self.channel)
#
...
#         return torch.mean((self.mindssc(x, delta=self.delta, sigma=self.sigma) - self.mindssc(y, delta=self.delta,
#                                                                                               sigma=self.sigma)) ** 2)
#
#
...
#             I = nn.functional.avg_pool3d(I, kernel_size=3, stride=2, padding=1, count_include_pad=False)
#             J = nn.functional.avg_pool3d(J, kernel_size=3, stride=2, padding=1, count_include_pad=False)
#             weight_map = nn.functional.avg_pool3d(weight_map, kernel_size=3, stride=2, padding=1,
...
class NCC(torch.nn.Module):
    ...
    def forward(self, I, J):
        ...
        weight = torch.ones((self.channel, 1, weight_win_size, weight_win_size, weight_win_size), device=I.device,
                            requires_grad=False)
        conv_fn = F.conv3d
...
#
#         weight_win_size = self.w_temp
#         weight = torch.ones((3, 1, weight_win_size, weight_win_size, weight_win_size), device=I.device,
#                             requires_grad=False)
#         conv_fn = F.conv3d
#
#         # compute CC squares
...
#              [[0., 0., 0.],
#               [0., 1., 0.],
#               [0., 0., 0.]]], requires_grad=False).to(device)
#
#         self.channel = 3
