...
class Dataset_epoch_NLST(Data.Dataset):
    ...
    def __getitem__(self, step):
        ...
        keypoints = []
        with open(self.keypoint_csv[step][0], newline='') as fixed_csvfile, open(self.keypoint_csv[step][1],
                                                                                 newline='') as moving_csvfile:
        ...
        if self.need_label:
            ...
            return torch.from_numpy(imgnorm(moving_img)).float(), torch.from_numpy(
                imgnorm(fixed_img)).float(), torch.from_numpy(moving_mask).float(), torch.from_numpy(
                fixed_mask).float(), keypoints
        else:
            return torch.from_numpy(imgnorm(moving_img)).float(), torch.from_numpy(
...
class Dataset_bratsreg_bidirection_all(Data.Dataset):
    ...
    def __init__(self, fixed_t1ce_list, moving_t1ce_list, fixed_flair_list, moving_flair_list, fixed_t2_list,
                 moving_t2_list, norm=True):
        'Initialization'
...
class Dataset_bratsreg_bidirection_all4(Data.Dataset):
    ...
    def __init__(self, fixed_t1_list, moving_t1_list, fixed_t1ce_list, moving_t1ce_list, fixed_flair_list,
                 moving_flair_list, fixed_t2_list, moving_t2_list, norm=True):
        'Initialization'
...
class Dataset_bratsreg_all_withmask(Data.Dataset):
    ...
    def __init__(self, fixed_t1ce_list, moving_t1ce_list, fixed_flair_list, moving_flair_list, fixed_t2_list,
                 moving_t2_list, tumor_list, norm=True):
        'Initialization'
    ...
    def __getitem__(self, step):
        ...
        if self.norm:
            return torch.from_numpy(imgnorm(moving_img)).float(), torch.from_numpy(
                imgnorm(fixed_img)).float(), torch.from_numpy(tumor_img).float()
        else:
            return torch.from_numpy(moving_img).float(), torch.from_numpy(fixed_img).float(), torch.from_numpy(
                tumor_img).float()
...
class Dataset_epoch_local(Data.Dataset):
    ...
    def __getitem__(self, step):
        ...
        if self.norm:
            return torch.from_numpy(imgnorm(moving_img)).float(), torch.from_numpy(
                imgnorm(fixed_img)).float(), torch.from_numpy(reg_seg).float()
        else:
            return torch.from_numpy(moving_img).float(), torch.from_numpy(fixed_img).float(), torch.from_numpy(
                reg_seg).float()
...
class Validation_Brats_all(Data.Dataset):
    def __init__(self, fixed_t1ce_list, moving_t1ce_list, fixed_flair_list, moving_flair_list, fixed_t2_list,
                 moving_t2_list, fixed_label_list, move_label_list, norm=True):
...
class Validation_Brats_all4(Data.Dataset):
    def __init__(self, fixed_t1_list, moving_t1_list, fixed_t1ce_list, moving_t1ce_list, fixed_flair_list,
                 moving_flair_list, fixed_t2_list, moving_t2_list, fixed_label_list, move_label_list, norm=True):
...
class Validation_Brats_all4_with_mask(Data.Dataset):
    def __init__(self, fixed_t1_list, moving_t1_list, fixed_t1ce_list, moving_t1ce_list, fixed_flair_list,
                 moving_flair_list, fixed_t2_list, moving_t2_list, fixed_label_list, move_label_list, tumor_list,
                 norm=True):
    ...
    def __getitem__(self, index):
        ...
        output = {'fixed': fixed_img.float(), 'move': moved_img.float(),
                  ...
                  'tumor_mask': tumor_img, 'index': index}
        return output


class Validation_Brats_all4_with_mask_submission(Data.Dataset):
    def __init__(self, fixed_t1_list, moving_t1_list, fixed_t1ce_list, moving_t1ce_list, fixed_flair_list,
                 moving_flair_list, fixed_t2_list, moving_t2_list, move_label_list, tumor_list, norm=True):
...
class Validation_Brats_all4_submission(Data.Dataset):
    def __init__(self, fixed_t1_list, moving_t1_list, fixed_t1ce_list, moving_t1ce_list, fixed_flair_list,
                 moving_flair_list, fixed_t2_list, moving_t2_list, move_label_list, norm=True):
    ...
    def __getitem__(self, index):
        ...
        output = {'fixed': fixed_img.float(), 'move': moved_img.float(), 'move_label': np.array(moved_key_list),
                  'index': index}
        return output


class Validation_Brats_t1cet2(Data.Dataset):
    def __init__(self, fixed_t1ce_list, moving_t1ce_list, fixed_t2_list, moving_t2_list, fixed_label_list,
                 move_label_list, norm=True):
...
class Validation_Brats_all_submission(Data.Dataset):
    def __init__(self, fixed_t1ce_list, moving_t1ce_list, fixed_flair_list, moving_flair_list, fixed_t2_list,
                 moving_t2_list, move_label_list, norm=True):
...
class Validation_Brats_with_mask(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        output = {'fixed': fixed_img.float(), 'move': moved_img.float(),
                  'fixed_label': np.array(fixed_key_list), 'move_label': np.array(moved_key_list),
                  'tumor_mask': torch.from_numpy(tumor_img).float(), 'index': index}
        return output
    ...
    def __getitem__(self, step):
        ...
        if self.need_label:
            ...
            return torch.from_numpy(imgnorm(insp_img)).float(), torch.from_numpy(
                imgnorm(exp_img)).float(), torch.from_numpy(insp_mask).float(), torch.from_numpy(
                exp_mask).float(), keypoints
        else:
            return torch.from_numpy(imgnorm(insp_img)).float(), torch.from_numpy(
